...
ffi.cdef [[
    ...
    unsigned long SFileGetMaxFileCount(uint32_t hMpq);
...
function archive:add_file(name, path)
    ...
    local c = self:number_of_files()
    if c >= self:get_max_file_count() - 2 then
        self:set_max_file_count(self:number_of_files() + 3)
    end
...
function archive:set_max_file_count(n)
    if self.handle == 0 then
        return false
    end
    return stormlib.SFileSetMaxFileCount(self.handle, n)
end

function archive:get_max_file_count()
    if self.handle == 0 then
        return 0
    end
    return stormlib.SFileGetMaxFileCount(self.handle)
end
