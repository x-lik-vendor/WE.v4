...
-- 根据ydwebase.dll的路径计算
fs.__ydwe_path = fs.get(fs.DIR_MODULE):parent_path():remove_filename():remove_filename()
...
function sys.spawn (command_line, current_dir, wait, hideUi, console_flag)
	local p = sys.process()
	if hideUi then
		p:hide_window()
	end
	if console_flag then
		p:set_console(console_flag)
	end
...
function sys.run (command_line, current_dir)
	local p = sys.process()
...
function sys.restart(map)
	local exe = fs.ydwe_path() / 'kkwe.exe'
	if map then
		if not sys.spawn(string.format('"%s" -loadfile "%s"', exe, map), fs.current_path()) then
			return
		end
	else
		if not sys.spawn(exe, fs.current_path()) then
			return
		end
	end
