...
jasshelper.exe_path_cli = jasshelper.path / "clijasshelper.exe"
	...
	if option.enable_jasshelper_scriptonly then
		return mpq_util:update_file(map_path, 'war3map.j',
			function (map_handle, in_script_path)
				...
				-- 执行并获取结果
				if not sys.spawn(command_line, fs.ydwe_path(), true, true) then
					return nil
				end
		...
		-- 执行并获取结果
		return sys.spawn(command_line, fs.ydwe_path(), true, true)
	end
...
function jasshelper.compile(self, map_path, option)
	...
	if self._lasterror then
		log.trace("Close last jasshelper errors dialog, pid", self._lasterror:id())
		self._lasterror:kill()
		self._lasterror:close()
		self._lasterror = nil
	end
	local res = self:do_compile(map_path, common_j_path, blizzard_j_path, option)
	local err_file = fs.ydwe_path() / 'logs' / 'compileerrors.txt'
	if fs.exists(err_file) then
		self._lasterror = sys.run(string.format('"%s" --showerrors', self.exe_path:string()), fs.ydwe_path())
		log.trace("Show jasshelper errors, pid", self._lasterror:id())
	end
