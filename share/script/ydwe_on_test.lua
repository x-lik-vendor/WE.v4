...
function event.EVENT_TEST_MAP(event_data)
    ...
    -- 未保存过的新建地图
    if not current_map_path then
        current_map_path = mappath
    end
    ...
    local useSlk = global_config["MapTest"]["EnableMapSlk"] ~= '0'
    if lniloader.is_lni(current_map_path) then
        if not lniloader.make_test(current_map_path, mappath, useSlk) then
            log.error('Pack map failed', current_map_path:string())
            return -1
        end
        log.info('Packed map success, use slk', useSlk)
    elseif useSlk then
        if not lniloader.make_test(current_map_path, mappath, true) then
            log.error('Pack map failed', current_map_path:string())
            return -1
        end
        log.info('Packed map success, use slk', useSlk)
    ...
    commandline = commandline .. ' -kkwe "' .. fs.ydwe_path():parent_path():string() .. '"'
