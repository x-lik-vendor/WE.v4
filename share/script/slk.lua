...
local function get_displayname(o1, o2)
    ...
    elseif o1._type == 'upgrade' then
        name = o1.name and o1.name[1] or o2.name and o2.name[1] or o1.name or o2.name or 'UNKNOWN_NAME'
    else
...
local function create_report()
    ...
    if #lold > 0 then
        ...
        for i = 1, math.min(10, #lold) do
            local o = old[lold[i]]
            lines[#lines + 1] = ("[%s][%s] '%s'"):format(displaytype[o._type],
                get_displayname(o, slk[o._type][o._parent]), o._id)
        end
    end
    if #lnew > 0 then
        if #lines > 0 then
    ...
    if #lines > 0 then
        table.insert(lines, 1, '编辑器刚刚帮你修改了物编数据,变化如下：')
...
function slk_proxy:refresh(mappath)
    ...
    for i, name in ipairs { 'ability', 'buff', 'unit', 'item', 'upgrade', 'doodad', 'destructable' } do
        if used[name] then
            local buf = w2l:backend_obj(name, obj[name])
            log.debug('refresh object: ' .. w2l.info.obj[name])
            ar:save_file(w2l.info.obj[name], buf)
            if type_map[w2l.info.obj[name]] then
                local tmp = fs.ydwe_path() / 'logs' / ('slk_' .. w2l.info.obj[name])
                if io.save(tmp, buf) then
                    we.import_customdata(type_map[w2l.info.obj[name]], tmp)
                    fs.remove(tmp)
                end
            end
        end
